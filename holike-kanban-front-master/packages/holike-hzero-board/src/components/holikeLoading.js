...
const HolikeLoading = ({ className }) => {
  useEffect(() => {
    const myChart = echarts.init(document.getElementById("holike-loading-chart"));
    const option = {
      series: [{
        type: 'liquidFill',
  ...
  return (
    <div className={className} id="holike-loading-chart" style={{ width: 80, height: 80 }} />
  );
};

const HolikeLoadingStyle = styled(HolikeLoading)`
  position: relative;
  transform: translateX(-50%);
  display: inline-block;
`;

export default React.createElement(HolikeLoadingStyle);