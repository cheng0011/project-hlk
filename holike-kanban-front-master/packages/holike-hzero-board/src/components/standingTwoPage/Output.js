...
const Chart = ({ className, chartId, chartDataList, showType }) => {
  ...
  chartDataList.forEach((item) => {
    ...
    // 目标产量
    const temp2 = showType === "area" ? Number(item.assistTargetDayOutput) : Number(item.uomTargetDayOutput);
    xData.push(`${Number(item.outputTime.split("-").pop())}日`);
    s1Data.push(temp1);
    s2Data.push(temp2);
